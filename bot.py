...
def parse_duration(text: str) -> int:
    """Parse duration string like 1s, 5m, 2h, 1d, 1w, 1mo into seconds."""
    ...
    try: return int(text)
    except: return 60  # default 1 minute

def format_duration(seconds: int) -> str:
    """Format seconds into human readable string."""
    if seconds < 60: return f'{seconds} second{"s" if seconds != 1 else ""}'
    if seconds < 3600: m = seconds // 60; return f'{m} minute{"s" if m != 1 else ""}'
    if seconds < 86400: h = seconds // 3600; return f'{h} hour{"s" if h != 1 else ""}'
    if seconds < 604800: d = seconds // 86400; return f'{d} day{"s" if d != 1 else ""}'
    if seconds < 2592000: w = seconds // 604800; return f'{w} week{"s" if w != 1 else ""}'
    mo = seconds // 2592000; return f'{mo} month{"s" if mo != 1 else ""}'

# ---------- Default Swear Lists ----------
# Soft swears — delete + warn
SOFT_SWEARS = [
    'damn', 'crap', 'hell', 'piss', 'ass', 'arse', 'bastard', 'bitch',
    'bloody', 'bollocks', 'bugger', 'cow', 'git', 'idiot', 'moron',
    'stupid', 'retard', 'dumbass', 'jackass', 'jerk', 'loser', 'shut up',
    'shutup', 'suck', 'sucker', 'twat', 'wanker', 'tosser'
]

# Hard swears — delete + mute 5 min
HARD_SWEARS = [
    'fuck', 'f u c k', 'fvck', 'f*ck', 'fuk', 'fucc',
    ...
    'go die', 'die bitch', 'die faggot'
]

def contains_word(content: str, word: str) -> bool:
    """Check if content contains word — exact or close match (handles spaces/numbers)."""
    import re
    clean = content.lower()
    # Direct substring match
    if word in clean: return True
    # Remove spaces and check
    if word.replace(' ','') in clean.replace(' ',''): return True
    # Word boundary check
    pattern = r'\b' + re.escape(word) + r'\b'
    if re.search(pattern, clean): return True
    return False
...
async def on_message(message):
    ...
    if settings.get('automod_enabled'):
        ...
        if settings.get('automod_badwords'):
            custom_words = settings.get('bad_words', [])
            use_default = settings.get('automod_default_swears', False)

            # Build word lists
            hard_words = list(HARD_SWEARS) if use_default else []
            soft_words = list(SOFT_SWEARS) if use_default else []
            # Add custom words to soft list
            soft_words.extend(custom_words)

            # Check hard swears first — mute 5 minutes
            hard_hit = next((w for w in hard_words if contains_word(content, w)), None)
            if hard_hit:
                await message.delete()
                # Auto mute 5 minutes
                try:
                    await message.author.timeout(datetime.timedelta(minutes=5), reason=f'Auto-mod: hard swear detected')
                except: pass
                # Add warn
                warns = load_json('warns.json')
                uid = str(message.author.id)
                if uid not in warns: warns[uid] = []
                warns[uid].append({'reason': f'Auto-mod: used a severe banned word', 'by': 'KPT_BOT', 'time': datetime.datetime.utcnow().isoformat()})
                save_json('warns.json', warns)
                await message.channel.send(
                    f'🔇 {message.author.mention} has been **muted for 5 minutes** and warned for using a severely banned word. '
                    f'Total warns: **{len(warns[uid])}**',
                    delete_after=10
                )
                add_log('AUTOMOD_HARD', f'Muted+warned {message.author} for hard swear', message.guild.id)
                deleted = True

            # Check soft swears — warn + delete
            if not deleted:
                soft_hit = next((w for w in soft_words if contains_word(content, w)), None)
                if soft_hit:
                    await message.delete()
                    # Add warn
                    warns = load_json('warns.json')
                    uid = str(message.author.id)
                    if uid not in warns: warns[uid] = []
                    warns[uid].append({'reason': f'Auto-mod: used a banned word', 'by': 'KPT_BOT', 'time': datetime.datetime.utcnow().isoformat()})
                    save_json('warns.json', warns)
                    await message.channel.send(
                        f'⚠️ {message.author.mention} watch your language! You have been warned. '
                        f'Total warns: **{len(warns[uid])}**',
                        delete_after=8
                    )
                    add_log('AUTOMOD_WORD', f'Warned {message.author} for soft swear', message.guild.id)
                    deleted = True
...
async def mute(ctx, member: discord.Member, duration: str='10m', *, reason='No reason provided'):
    ...
    embed.add_field(name='User',value=str(member)); embed.add_field(name='Duration',value=human); embed.add_field(name='Reason',value=reason)
    await ctx.send(embed=embed); add_log('MUTE', f'{ctx.author} muted {member} for {human} | {reason}', ctx.guild.id)
...
async def giveaway(ctx, duration: str, winners: int, *, prize: str):
    ...
    add_log('GIVEAWAY_START', f'{ctx.author} started: {prize} ({human})', ctx.guild.id)
...
@app_commands.describe(member='The member to mute', duration='Duration e.g. 10m, 1h, 2d, 1w', reason='Reason')
async def slash_mute(interaction: discord.Interaction, member: discord.Member, duration: str='10m', reason: str='No reason provided'):
    ...
    await interaction.response.defer()
    seconds = min(parse_duration(duration), 2419200)
    human = format_duration(seconds)
    await member.timeout(datetime.timedelta(seconds=seconds), reason=reason)
    embed = discord.Embed(title=m('moderation','mute_title'), color=color('mute'), timestamp=datetime.datetime.utcnow())
    embed.add_field(name='User',value=str(member)); embed.add_field(name='Duration',value=human); embed.add_field(name='Reason',value=reason)
    await interaction.followup.send(embed=embed); add_log('MUTE', f'{interaction.user} muted {member} for {human} | {reason}', interaction.guild.id)
...
@app_commands.describe(duration='How long e.g. 1h, 30m, 2d, 1w', winners='Number of winners', prize="What you're giving away")
async def slash_giveaway(interaction: discord.Interaction, duration: str, winners: int, prize: str):
    ...
    save_json('giveaways.json', gws)
    add_log('GIVEAWAY_START', f'{interaction.user} started: {prize} ({human})', interaction.guild.id)
    await asyncio.sleep(seconds)
    ...
    for action in actions:
        try:
            ...
            if atype == 'announce':
                ...
                ch = guild.get_channel(int(data.get('channel_id', 0)))
                if ch:
                    duration = data.get('duration', data.get('minutes', '60m'))
                    seconds = parse_duration(str(duration)) if not str(duration).isdigit() else int(duration)*60
                    human = format_duration(seconds)
                    ...
                    save_json('giveaways.json', gws)
                    add_log('GIVEAWAY_START', f'Dashboard started: {prize} ({human})', guild.id)
